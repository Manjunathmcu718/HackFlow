...
import { ArrowRight, Users, Trophy, Globe, Zap, Sparkles, Star, CheckCircle2 } from "lucide-react";
...
function AnimatedStat({ val, label, color, delay }) {
  const suffix = String(val).replace(/[0-9.,]/g, "");
  const { count, ref } = useCountUp(val, 1800, false);
  const numericVal = parseFloat(String(val).replace(/[^0-9.]/g, ""));
  const displayVal = isNaN(numericVal) ? val : `${count.toLocaleString()}${suffix}`;

  return (
    <motion.div ref__={ref}
      initial={{ opacity:0, scale:.85 }} animate={{ opacity:1, scale:1 }} transition={{ delay }}
      ...
      <div className="w-10 h-10 rounded-xl overflow-hidden mb-1 flex items-center justify-center"
        ...
        <img src="https://media.base44.com/images/public/6a305537a00d359b98abca78/f738bf2e7_image.png"
          alt="BeetleX" className="w-8 h-8 object-contain" />
      </div>
      <div className="font-heading font-extrabold text-2xl" style={{ color:"#1A1F3C" }}>{displayVal}</div>
...
function MagBtn({ children }) {
  ...
  return (
    <motion.div ref__={ref} style={{ x:sx, y:sy }}
      onMouseMove={e => { const r = ref.current?.getBoundingClientRect(); if(!r) return; x.set((e.clientX-r.left-r.width/2)*.28); y.set((e.clientY-r.top-r.height/2)*.28); }}
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:.6 }}
              ...
              <img src="https://media.base44.com/images/public/6a305537a00d359b98abca78/f738bf2e7_image.png" alt="BeetleX" className="w-4 h-4 object-contain rounded-full" />
              Registration Open Â· {hackathon?.participant_count?.toLocaleString() || "2,847"}+ builders
            </motion.div>

            <motion.h1 initial={{ opacity:0, y:40 }} animate={{ opacity:1, y:0 }} transition={{ duration:.85, delay:.1, ease:[.22,1,.36,1] }}
              className="font-heading font-extrabold tracking-tight leading-[1.0] mb-6"
              style={{ fontSize:"clamp(3rem,6vw,5.2rem)", color:"#1A1F3C" }}>
              Build the{" "}
              <span className="text-grad-primary">Future.</span>
              <br />
              <span className="text-grad-violet">Win Big.</span>
            </motion.h1>

            <motion.div className="h-1.5 w-32 rounded-full mb-6"
              style={{ background:"linear-gradient(90deg,#F4622A,#7C4DFF)" }}
              initial={{ scaleX:0, originX:0 }} animate={{ scaleX:1 }} transition={{ delay:1, duration:.8 }} />
            ...
            <motion.p initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:.3, duration:.7 }}
              ...
              {" "}2 weeks Â· $25K+ in prizes Â· 80 countries.
            </motion.p>

            <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:.45, duration:.6 }}
            ...
            <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:.6, duration:.6 }}
          ...
          <motion.div initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }} transition={{ delay:.35, duration:.85, ease:[.22,1,.36,1] }}
            className="flex flex-col gap-5">
            <div className="card-gradient rounded-3xl p-7"
              style={{ background:"linear-gradient(135deg,#fff 0%,#FFF5EF 100%)" }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {STATS.map(({ val, label, color }, i) => (
                <AnimatedStat key={label} val={val} label={label} color={color} delay={.5+i*.08} />
              ))}
            </div>
          </motion.div>
        </div>
