...
const defaultPrizes = [
  { place:"Grand Prize",       amount:"$10,000", description:"Best overall project across all tracks",    icon:Trophy, bg:"linear-gradient(135deg,#F4622A,#FB923C,#FDE68A)", tag:"#F4622A", label:"ðŸ† Top Prize" },
  { place:"1st Place / Track", amount:"$5,000",  description:"Top project in each technology track",      icon:Award,  bg:"linear-gradient(135deg,#7C4DFF,#A855F7,#C084FC)", tag:"#7C4DFF", label:null },
  { place:"Best UI/UX",        amount:"$2,500",  description:"Most polished and user-friendly interface", icon:Star,   bg:"linear-gradient(135deg,#EC4899,#F4622A,#FB923C)", tag:"#EC4899", label:null },
  { place:"Community Choice",  amount:"$2,500",  description:"Voted by fellow participants",               icon:Heart,  bg:"linear-gradient(135deg,#10B981,#06B6D4,#3B82F6)", tag:"#10B981", label:null },
];

const trackIcons = [Cpu, Globe, Gamepad2, Wrench];

export default function PrizesSection({ hackathon }) {
  const prizes = hackathon?.prizes?.length > 0
    ? hackathon.prizes.map((p, i) => ({ ...defaultPrizes[i % defaultPrizes.length], ...p }))
    : defaultPrizes;
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <motion.h2 initial={{ opacity:0, y:16 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:.1 }}
            ...
            Over <span className="text-grad-primary">$25,000</span> in prizes
          </motion.h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-16">
          {prizes.map((p, i) => (
            <motion.div key={p.place}
              ...
              whileHover={{ y:-10, scale:1.03, transition:{ duration:.2 } }}
              ...
              <div className="h-32 relative" style={{ background:p.bg }}>
                ...
                <div className="absolute bottom-4 right-4">
                  <p.icon className="w-10 h-10 text-white" style={{ opacity:.7 }} />
                </div>
              </div>
          ...
          <motion.div initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {hackathon.tracks.map((t, i) => {
                const TIcon = trackIcons[i % trackIcons.length];
                return (
                  <motion.div key={t.name}
                    initial={{ opacity:0, x:i%2===0?-16:16 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}
                    transition={{ delay:i*.08 }} whileHover={{ x:4 }}
                    className="card-light flex items-start gap-4 p-5 rounded-2xl">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                      style={{ background:"rgba(244,98,42,.08)" }}>
                      <TIcon style={{ width:18, height:18, color:"#F4622A" }} />
                    </div>
