...
export default function RegistrationHero({ hackathonTitle }: { hackathonTitle?: string }) {
  return (
    <div className="registration-hero relative overflow-hidden pt-28 pb-10"
      ...
      <div className="registration-hero-content relative max-w-2xl mx-auto px-4 sm:px-6">
        ...
        <h1 className="registration-hero-title font-heading font-extrabold tracking-tight" style={{ fontSize:"clamp(2rem,5vw,3.2rem)",color:"#1A1F3C" }}>
          Register for <span className="text-shimmer">Hackathon</span>
        </h1>
        <p className="registration-hero-subtitle mt-2 text-base font-semibold" style={{ color:"rgba(26,31,60,.5)" }}>{hackathonTitle}</p>
      </div>
    </div>
  );
}
