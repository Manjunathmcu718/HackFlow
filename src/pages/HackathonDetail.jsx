...
import { Users, Trophy, Clock, Code2, ArrowRight, BookOpen, Shield, Plus, Minus } from "lucide-react";
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm" style={{ color:"rgba(26,31,60,.5)" }}>
              {hackathon.start_date && format(new Date(hackathon.start_date),"MMM d")} -- {hackathon.end_date && format(new Date(hackathon.end_date),"MMM d, yyyy")}
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6 mb-10">
            <div className="flex items-center gap-2 text-sm"><Users className="w-4 h-4" style={{ color:"#F4622A" }} /><span className="font-semibold" style={{ color:"#1A1F3C" }}>{hackathon.participant_count?.toLocaleString()}</span><span style={{ color:"rgba(26,31,60,.5)" }}>participants</span></div>
            <div className="flex items-center gap-2 text-sm"><Trophy className="w-4 h-4" style={{ color:"#F4622A" }} /><span className="font-semibold" style={{ color:"#1A1F3C" }}>{hackathon.team_count}</span><span style={{ color:"rgba(26,31,60,.5)" }}>teams</span></div>
            <div className="flex items-center gap-2 text-sm"><Code2 className="w-4 h-4" style={{ color:"#F4622A" }} /><span className="font-semibold" style={{ color:"#1A1F3C" }}>{hackathon.tracks?.length || 0}</span><span style={{ color:"rgba(26,31,60,.5)" }}>tracks</span></div>
            <div className="hidden md:block ml-auto">
              <Link to="/register-hackathon">
                <button className="btn-primary flex items-center gap-2 px-8 py-3 text-sm">Register Now <ArrowRight className="w-4 h-4" /></button>
              </Link>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl mb-10 flex flex-col sm:flex-row items-center justify-between gap-6"
            ...
            <div className="flex items-center gap-2 text-sm" style={{ color:"rgba(26,31,60,.5)" }}>
              <Clock className="w-4 h-4" /><span>Teams: {hackathon.min_team_size}-{hackathon.max_team_size} members</span>
            </div>
          </div>

          {/* Description */}
          <div className="mb-10">
            <div className="flex items-center gap-2 mb-3"><BookOpen className="w-5 h-5" style={{ color:"#F4622A" }} /><h2 className="font-heading text-xl font-bold" style={{ color:"#1A1F3C" }}>About This Hackathon</h2></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            {hackathon.rules && (
              <div className="p-5 rounded-2xl" style={{ background:"#fff", border:"1px solid rgba(26,31,60,.08)" }}>
                <div className="flex items-center gap-2 mb-3"><Shield className="w-4 h-4" style={{ color:"#F4622A" }} /><h3 className="font-semibold text-sm" style={{ color:"#1A1F3C" }}>Rules</h3></div>
                <p className="text-sm leading-relaxed" style={{ color:"rgba(26,31,60,.6)" }}>{hackathon.rules}</p>
              </div>
            )}
            {hackathon.eligibility && (
              <div className="p-5 rounded-2xl" style={{ background:"#fff", border:"1px solid rgba(26,31,60,.08)" }}>
                <div className="flex items-center gap-2 mb-3"><Users className="w-4 h-4" style={{ color:"#F4622A" }} /><h3 className="font-semibold text-sm" style={{ color:"#1A1F3C" }}>Eligibility</h3></div>
