...
import { Check, ArrowRight, ArrowLeft, Copy, Share2, User, Users, Code2, Eye, Sparkles } from "lucide-react";
...
const FL = ({ children }) => <label className="block text-sm font-bold mb-2" style={{ color:"#1A1F3C" }}>{children}</label>;
const FI = ({ ...props }) => (
  <input {...props}
...
export default function RegisterHackathon() {
  ...
  const submit = () => {
    const id = "BX-" + Date.now().toString(36).toUpperCase();
    setRegId(id);
    setDone(true);
  };

  const Err = ({ field }) => errors[field] ? <p className="text-xs font-semibold mt-1.5" style={{ color:"#F43F5E" }}>{errors[field]}</p> : null;
    ...
    <PageShell>
      <div className="relative min-h-screen flex items-center justify-center overflow-hidden"
        ...
        <motion.div initial={{ opacity:0,scale:.85 }} animate={{ opacity:1,scale:1 }}
          ...
          <h1 className="font-heading text-4xl font-extrabold mb-3" style={{ color:"#1A1F3C" }}>You're In! ðŸŽ‰</h1>
          ...
          <div className="p-5 rounded-2xl mb-6"
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <button onClick={() => navigator.clipboard.writeText(regId)}
  ...
  return (
    <PageShell>
      ...
      <div className="relative overflow-hidden pt-28 pb-10"
        ...
        <Orb style={{ width:380,height:380,bottom:"-60px",left:"-80px",background:"radial-gradient(ellipse,rgba(124,77,255,.13) 0%,transparent 70%)",filter:"blur(60px)",dur:12,del:2 }} />
        <motion.div className="absolute top-12 right-28 w-20 h-20 rounded-full border-2 pointer-events-none"
          style={{ borderColor:"rgba(244,98,42,.2)", borderStyle:"dashed" }}
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 pb-20">
            ...
            return (
              <React.Fragment key={s}>
                <div className="flex items-center gap-2">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all flex-shrink-0"
                    style={{
            ...
            <div className="rounded-3xl p-6 sm:p-8"
              ...
              {step === 0 && (
                <div className="space-y-5">
                  <div><FL>Full Name *</FL><FI value={form.participant_name} onChange={e => update("participant_name",e.target.value)} placeholder="John Doe" /><Err field="participant_name" /></div>
                  <div><FL>Email Address *</FL><FI type="email" value={form.email} onChange={e => update("email",e.target.value)} placeholder="john@example.com" /><Err field="email" /></div>
                  <div><FL>College / Organization *</FL><FI value={form.organization} onChange={e => update("organization",e.target.value)} placeholder="MIT" /><Err field="organization" /></div>
                  <div><FL>Your Role *</FL><FI value={form.role_title} onChange={e => update("role_title",e.target.value)} placeholder="Frontend Developer" /><Err field="role_title" /></div>
                </div>
              )}
              ...
              {step === 1 && (
                <div className="space-y-4">
                  ...
                  {form.team_action==="create" && <div><FL>Team Name *</FL><FI value={form.team_name} onChange={e => update("team_name",e.target.value)} placeholder="ChainBreakers" /><Err field="team_name" /></div>}
                  {form.team_action==="join"   && <div><FL>Invite Code *</FL><FI value={form.invite_code} onChange={e => update("invite_code",e.target.value)} placeholder="CB-2025-X7K" /><Err field="invite_code" /></div>}
                </div>
              )}

              {step === 2 && (
                <div>
                  <FL>Select a Technology Track *</FL>
              ...
              {step === 3 && (
                <div>
                  <h3 className="font-heading font-extrabold text-xl mb-5" style={{ color:"#1A1F3C" }}>Review Your Registration</h3>
                  <div className="rounded-2xl overflow-hidden" style={{ border:"1px solid rgba(26,31,60,.09)" }}>
                    {[
                      ...
                      <div key={item.label} className="flex items-center justify-between px-4 py-3"
                        ...
                        <span className="text-xs font-bold uppercase tracking-wider" style={{ color:"rgba(26,31,60,.4)" }}>{item.label}</span>
                        <span className="text-sm font-semibold" style={{ color:"#1A1F3C" }}>{item.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          {step < 3 ? (
            ...
            <button onClick={submit} className="btn-primary flex items-center gap-2 px-8 py-3 text-sm font-semibold">
              Confirm Registration <Check className="w-4 h-4" />
            </button>
          )}
        </div>
      </div>
